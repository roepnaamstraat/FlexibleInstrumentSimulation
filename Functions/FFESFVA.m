function [time,sig,f] = FFESFVA(t,ne,le,e,ep,nx,lnp,x,xp)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FORCE INPUT - FLEXIBLE ANALYSIS OF CATHETER IN S-SHAPED CHANNEL 
...
% Preallocation
fx  = [(1:2:47)',1*ones(24,1),zeros(24,1)];   
fy  = [(1:2:47)',2*ones(24,1),zeros(24,1)]; 
fz  = [(1:2:47)',3*ones(24,1),zeros(24,1)]; 
   
M1  = [(2:2:48)',1*ones(24,1),zeros(24,1)]; 
M2  = [(2:2:48)',2*ones(24,1),zeros(24,1)];    
M3  = [(2:2:48)',3*ones(24,1),zeros(24,1)]; 
M4  = [(2:2:48)',4*ones(24,1),zeros(24,1)];  
...
for i = 1:24
    ...
    pos       = x(lnp(node,1:3));    %contains x,y,z coordinate
    pos2      = x(lnp((i+1)*2-1,1:3));
    ...
    qp        = xp(lnp(node+1,:));   %contains angular velocity in terms of Euler parameters 
    qtilde    = [ 0   -q(4)  q(3);
                  q(4) 0    -q(2);
                 -q(3) q(2)  0   ];
    Q_bar     = [q(1)    -q(2:4);
